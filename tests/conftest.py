...
def stub_subtopic_page(db_session, stub_topic_page):
    ...
    page.page_json = json.dumps(
        {"guid": "subtopic_example", "title": "Test subtopic page"}
    )
...
def stub_type_of_statistic(db_session):
    type_of_statistic = TypeOfStatistic(
        id=1, internal="National", external="National", position=1
    )
...
def mock_create_page(mocker, stub_measure_page):
    ...
    return mocker.patch(
        "application.cms.views.page_service.create_page", side_effect=_create_page
    )
...
def mock_get_page(mocker, stub_topic_page, stub_measure_page):
    ...
    return mocker.patch(
        "application.cms.views.page_service.get_page", side_effect=_get_page
    )
...
def mock_get_measure_page(mocker, stub_measure_page):
    return mocker.patch(
        "application.cms.views.page_service.get_page", return_value=stub_measure_page
    )
...
def mock_reject_page(mocker, stub_topic_page):
    return mocker.patch(
        "application.cms.views.page_service.reject_page", return_value=stub_topic_page
    )
...
@pytest.fixture(scope="function")
def stub_page_with_dimension_and_chart_and_table(
    db_session, stub_page_with_dimension_and_chart
):
...
def mock_get_measure_download(mocker):
    ...
    return mocker.patch(
        "application.static_site.views.upload_service.get_measure_download",
        side_effect=get,
    )
...
def mock_get_content_with_metadata(mocker):
    return mocker.patch(
        "application.static_site.views.get_content_with_metadata",
        return_value="i do not care",
    )
